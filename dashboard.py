...
carburant_options = [{'label': carburant, 'value': carburant} for carburant in df['nom_carburant'].unique()]
...
@app.callback(
    [Output('prix-moyen-graph', 'figure'), Output('carburant-color-rectangle', 'style')]
    ,
...
def update_graph(selected_carburant):
    ...
    fig = px.line(mois_moyen, x='mois', y='valeur_carburant', 
                     ...
                     color_discrete_sequence=[couleursCarburant[selected_carburant][0]],
                     )
